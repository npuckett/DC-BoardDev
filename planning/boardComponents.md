...
#### Power Output
This should be able to be done with transistor circuits that go off of the PWM outputs of the board. I have somee off the shelf PWM to 10V boards that work OK that you can have a look at. The main issues I see:
- Having safeguards to ensure that output max is 10V even though input is 12V. This could be done with software clipping, but I'd rather build it into the hardware.
- Components to get clean 0V at the bottom end. I had issues with the current setup with it not going fully to 0V to shut the panels completely off. My circuit was pretty simple, but having things to keep this reliable is important.
#### Connectors
...
# Open Questions
## Number of Outputs
In my mind this is somewhere between 4 - 8, depending on the form factor of the board , general complexity, etc

## The 0-10V output circuit
The simple choice seems to be 
- MOSFET based circuit
There is also an argument to use a DAC chip since we are talking about outputing analog voltage.
- MCP4728 outputs 4 outputs, controlled over I2C
...
| MOSFET-based output circuit | Potential | 0-10V voltage control | Simple approach; needs hardware clamping at 10V and clean 0V shutoff |
| MCP4728 DAC (I2C, 4-ch) | Potential | 0-10V voltage control | Cleaner analog output; limits channels to 4 per chip |
